...
class Load
{
    ...
    public function addOne(int $id, $need_pinyin = 0, $need_queue = 0) {
        if ($need_queue == 1) {
            ImportJob::dispatch($this->db_class, $this->columns, $id, $need_pinyin, 0)->onQueue("n2_build");
            return;
        }
    ...
    public function addBatch($need_pinyin = 0, $need_queue = 0) {
        ...
        while(1) {
            ...
            if ($need_queue == 1) {
                ImportJob::dispatch($this->db_class, $this->columns, 0, $need_pinyin, 1, $log)->onQueue("n2_build");
                continue;
            }
